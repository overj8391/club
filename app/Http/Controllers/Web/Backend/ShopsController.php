...
namespace VanguardLTE\Http\Controllers\Web\Backend
{
    ...
    class ShopsController extends \VanguardLTE\Http\Controllers\Controller
    {
        ...
        public function edit($shop)
        {
            ...
            //
            $game_bank = \VanguardLTE\GameBank::where('shop_id', $shop->id)->first();
            $game_bank_slots = round($game_bank->slots);
            $game_bank_bonus = round($game_bank->bonus);
            //

            return view('backend.shops.edit', compact('shop', 'directories', 'categories', 'cats', 'blocks', 'activity', 'game_bank_slots', 'game_bank_bonus'));
        }
        public function update(\Illuminate\Http\Request $request, \VanguardLTE\Repositories\Session\SessionRepository $sessionRepository, \VanguardLTE\Shop $shop)
        {
            ...
            //
            $game_bank_data = $request->only(['game_bank_slots', 'game_bank_bonus']);
            if (is_numeric($game_bank_data['game_bank_slots']) && $game_bank_data['game_bank_slots'] >= 0) {
                $game_bank_slots = $game_bank_data['game_bank_slots'] <= $shop->shop_limit ? round($game_bank_data['game_bank_slots']) : $shop->shop_limit;
                \VanguardLTE\GameBank::where('shop_id', $shop->id)->update(['game_bank.slots' => $game_bank_slots, 'game_bank.table_bank' => $game_bank_slots]);
            }
            if (is_numeric($game_bank_data['game_bank_bonus']) && $game_bank_data['game_bank_bonus'] >= 0) {
                $game_bank_bonus = $game_bank_data['game_bank_bonus'] <= $shop->shop_limit ? round($game_bank_data['game_bank_bonus']) : $shop->shop_limit;
                \VanguardLTE\GameBank::where('shop_id', $shop->id)->update(['game_bank.bonus' => $game_bank_bonus]);
            }
            //
