...
namespace VanguardLTE\Http\Controllers\Web\Backend
{
    ...
    class DashboardController extends \VanguardLTE\Http\Controllers\Controller
    {
        ...
        public function money_stat(\Illuminate\Http\Request $request)
        {
            $users = auth()->user()->hierarchyUsers();
            $statistics = \VanguardLTE\Statistic::select('statistics.id')->orderBy('statistics.id', 'DESC');

            $statistics = $statistics->where('statistics.system', 'handpay');

            if (auth()->user()->shop_id > 0)
            {
                $statistics = $statistics->whereIn('statistics.shop_id', auth()->user()->availableShops());
            }


            if ($request->user != '')
            {
                $statistics = $statistics->join('users', 'users.id', '=', 'statistics.user_id');
                $statistics = $statistics->where('users.username', 'like', '%' . $request->user . '%');
            }
            if ($request->type != '')
            {
                $statistics = $statistics->where('statistics.type', $request->type);
            }
            if ($request->sum_from != '')
            {
                $statistics = $statistics->where('statistics.sum', '>=', $request->sum_from);
            }
            if ($request->sum_to != '')
            {
                $statistics = $statistics->where('statistics.sum', '<=', $request->sum_to);
            }
            ...
            {
                $dates = explode(' - ', $request->dates);
                $statistics = $statistics->where('statistics.created_at', '>=', $dates[0]);
                $statistics = $statistics->where('statistics.created_at', '<=', $dates[1]);

                $filter_date_from = $dates[0];
                $filter_date_to = $dates[1];
            }
            if ($request->shift_time_hours != '')
            {
                $curr_data = date('Y-m-d H:m', strtotime('-'. $request->shift_time_hours .' hours'));
                $statistics = $statistics->where('statistics.created_at', '>=', $curr_data);

                $filter_date_from = $curr_data;
            }
            ...
            $statistics = $statistics->offset($offset)->take($perPage)->pluck('id');
            $time_elapsed_secs = microtime(true) - $start;
            $statistics_stat = \VanguardLTE\Statistic::whereIn('id', $statistics)->orderBy('id', 'DESC')->get();
            ...
            return view('backend.stat.money_stat', compact(
                'statistics_stat', 'filter_date_from', 'filter_date_to', 'stat_add', 'stat_out', 'count', 'page', 'perPage'
            ));
        }
        public function money_stat_clear(\Illuminate\Http\Request $request) {
            $shop_id = auth()->user()->shop_id;
            \VanguardLTE\Statistic::where('statistics.system', 'handpay')->update(['statistics.system' => 0, 'statistics.title' => 'HP_del']);
            \VanguardLTE\Game::where('games.shop_id', $shop_id)->update(['games.bids' => 0, 'games.stat_in' => 0, 'games.stat_out' => 0]);
            return redirect()->route('backend.money_stat');
        }
