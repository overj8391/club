...
namespace VanguardLTE\Http\Controllers\Web\Frontend
{
    ...
    {
        public function index(\Illuminate\Http\Request $request, $category = '')
        {
            if (!$category) {
                return redirect()->route('frontend.game.list.category', 'all');
            }
            ...
            \Illuminate\Support\Facades\Cookie::queue('currentCategory' . (\Illuminate\Support\Facades\Auth::check() ? auth()->user()->id : 0), $category, 2678400);
            if( $category != '' )
            {
            ...
            if( $games )
            {
                ...
                if( count($cat_ids) )
                {
                    ...
                    $categories = \VanguardLTE\Category::whereIn('href', [
                        'betsoft',
                        'igrosoft',
                        'netent',
                        'playngo',
            ...
            return view('frontend.Default.games.list', compact(
                'games',
                'categories',
                'currentSliderNum'
            ));
